...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'starstruck':
				dialogue = CoolUtil.coolTextFile(Paths.txt('starstruck/starstruckDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'senpai' | 'roses' | 'supreme':
			{
					...
					bgGirls = new BackgroundGirls(-10000, 19000);
					...
					if (SONG.song.toLowerCase() == 'supreme')
					{
						bgGirls = new BackgroundGirls(-100, 190);
						bgGirls.scrollFactor.set(0.9, 0.9);
							bgGirls.getScared();
			...
			case 'starstruck':
			{
					...
					var bg:FlxSprite = new FlxSprite(posX, posY);
					bg.frames = Paths.getSparrowAtlas('weeb/animatedEvilSchool2');
					bg.animation.addByPrefix('idle', 'background 2', 24);
					bg.animation.play('idle');
					bg.scrollFactor.set(0.8, 0.9);
					bg.scale.set(7, 7);
					...
					/* 
							var bg:FlxSprite = new FlxSprite(posX, posY).loadGraphic(Paths.image('weeb/evilSchool2BG'));
							bg.scale.set(6, 6);
							// bg.setGraphicSize(Std.int(bg.width * 6));
							// bg.updateHitbox();
							add(bg);
							var fg:FlxSprite = new FlxSprite(posX, posY).loadGraphic(Paths.image('weeb/evilSchool2FG'));
							fg.scale.set(6, 6);
							// fg.setGraphicSize(Std.int(fg.width * 6));
							// fg.updateHitbox();
							add(fg);
		...
		switch (SONG.player2)
		{
			...
			case 'starpai':
				dad.x -= 150;
				dad.y += 100;
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y + 400);
			case 'drippai':
				dad.x += -40;
				dad.y += 80;
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				case 'school' | 'schoolEvil' | 'schoolEvil2':
